...
class NewsCategoryClassifier:
    def __init__(self, config: dict) -> None:
        ...
        featurizer = TransformerFeaturizer(
            ...
            SentenceTransformer(
                "sentence-transformers/{model}".format(model=self.config["model"]["featurizer"]["sentence_transformer_model"])
                )
            )
        
        model = joblib.load(
            self.config["model"]["classifier"]["serialized_model_path"]
            )
    ...
    def predict_proba(self, model_input: dict) -> dict:
        ...
        return dict(
            zip(
                self.labels,
                self.pipeline.predict_proba([model_input.description]).squeeze()
            )
        )
...
def predict(request: PredictRequest):
    ...
    predicted_probas = classifier.predict_proba(request)
    ...
    log_data = {
        ...
        "prediction": {"label": predicted_label, "scores": predicted_probas},
        "latency": latency,
    }
